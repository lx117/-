	...
	private static boolean applyIptablesRulesImpl(Context ctx, List<Integer> uidsWifi, List<Integer> uids3g, boolean showErrors) {
		...
		try {
			...
			if (any_wifi) {
				...
				script.append("" +
						/*"iptables -A droidwall-wifi -p tcp -m string --string Host: --algo bm -j MARK --set-mark 1 || exit\n" +
						"$IPTABLES -A FORWARD -p tcp -m string --algo bm --string \"baidu.com\" -j ACCEPT  || exit 1\n" +
						"$IPTABLES -A FORWARD -p udp -m string --algo bm --string \"baidu.com\" -j ACCEPT || exit 2\n" +
						"$IPTABLES -A FORWARD -p udp -m multiport --dport 53 -j DROP || exit 3\n" +
						"$IPTABLES -A OUTPUT -p tcp -m string --algo bm --string \"baidu.com\" -j ACCEPT || exit 4\n" +
						"$IPTABLES -A OUTPUT -p udp -m string --algo bm --string \"baidu.com\" -j ACCEPT || exit 5\n" +
						"$IPTABLES -A OUTPUT -p udp -m multiport --dport 53 -j DROP || exit 6\n" +*/
						"");
			}
			if (whitelist) {
				if (!any_3g) {
	...
	public static DroidApp[] getApps(Context ctx) {
		...
		try {
			...
			final DroidApp special[] = {
				...
				new DroidApp(android.os.Process.getUidForName("root"), "(root) - Applications running as root", true, true),
